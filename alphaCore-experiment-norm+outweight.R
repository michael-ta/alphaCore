  ...
  aCore<-function(tokenGr, edgelist, depthInputData, alphaCoreMap,step=0.01){
    ...
    depthInputData <- cbind(depthInputData, 0, 0, 0, 1) 
    ...
    while(alpha > 0){
      ...
      # normalized the degree - we've already normalize the weights
	    depthInputData[,6] = depthInputData[,3]
         
	    depthInputData[,5] = (depthInputData[,2] - min(depthInputData[,2])) / 
                             (max.degree - min(depthInputData[,2]))

        depthInputData[,7] = depthInputData[,4] 
      ...
      nodesToBeDeleted <- which(depthInputData[,8] >= level)
     ...
     if(updated==FALSE){
        ...
        delta <- FindNextAlpha(alpha, depthInputData[,8]) / step
        ...
        for (v in nodesToBeDeleted) {
          ...
          if (!(is.null(nrow(depthInputData)))) {
              ...
              for (anode in edgelist[eidx2, 1]) {
                  aidx <- which(depthInputData[,1] == anode)
                  widx <- which(edgelist[eidx2,1] == anode)
                  depthInputData[aidx,4] = depthInputData[aidx,4] - 
                                           edgelist[eidx2,3][widx]
              }
  ...
  initialNodeFeatures <- lapply(as.integer(vertex_attr(tokenGr)$idx),
                                FUN=function(x) {
                                nidx <- which(data[,2] == x)
                                return( c(x, length(nidx), sum(data[nidx,3])) ) })
  initialNodeFeatures <- do.call(rbind, initialNodeFeatures) 
  data[,3] = (1 + (data[,3] * (max(initialNodeFeatures[,2]) - 1))) / max(initialNodeFeatures[,2])

  # should be faster to iterate though the dataset
  initialNodeFeatures[,3] = sapply(as.integer(vertex_attr(tokenGr)$idx),
                                   FUN=function(x) {
                                   nidx <- which(data[,2] == x)
                                   return( sum(data[nidx,3]) ) }) 
  initialNodeFeatures = cbind(initialNodeFeatures, 
                              sapply(as.integer(vertex_attr(tokenGr)$idx),
                                     FUN=function(x) {
                                     nidx <- which(data[,1] == x)
                                     return( sum(data[nidx,3]) ) }))
  colnames(initialNodeFeatures)<-c("node","inDegree","inWeight", "outweight")
